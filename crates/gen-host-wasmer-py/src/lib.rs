...
pub struct WasmerPy {
    ...
    deps: Dependencies,
    /// Whether the Python Union being emitted will wrap its cases with dataclasses
...
impl Opts {
    pub fn build(self) -> WasmerPy {
        let mut r = WasmerPy::new();
        r.opts = self;
        r
    }
}
...
impl Generator for WasmerPy {
    ...
    fn export(&mut self, iface: &Interface, func: &Function) {
        ...
        builder.push_str(&format!("def {}(", func.name.to_snake_case(),));
        ...
        if needs_memory {
            ...
            builder.push_str("m = get_export(\"memory\")\n");
            builder.push_str("assert(isinstance(m, wasmer.Memory))\n");
            builder.deps.pyimport("typing", "cast");
            builder.push_str("memory = cast(wasmer.Memory, m)\n");
            locals.insert("memory").unwrap();
        }

        if let Some(name) = needs_realloc {
            builder.push_str(&format!("realloc = get_export(\"{}\")\n", name));
            builder.push_str("assert(isinstance(realloc, wasmer.Function))\n");
            locals.insert("realloc").unwrap();
        }

        if let Some(name) = needs_free {
            builder.push_str(&format!("free = get_export(\"{}\")\n", name));
            builder.push_str("assert(isinstance(free, wasmer.Function))\n");
    ...
    fn finish_one(&mut self, iface: &Interface, files: &mut Files) {
        ...
        self.deps.pyimport("typing", "Callable");
...
impl FunctionBindgen<'_> {
    fn new(gen: &mut WasmerPy, params: Vec<String>) -> FunctionBindgen<'_> {
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::StringLift { free, .. } => {
                ...
                let result = format!("_decode_utf8(memory, {}, {})", ptr, len);
                match free {
                    Some(free) => {
                        self.needs_free = Some(free.to_string());
                        let list = self.locals.tmp("list");
                        builder.push_str(&format!("{} = {}\n", list, result));
                        self.src.push_str(&format!("free({}, {}, 1)\n", ptr, len));
                        results.push(list);
                    }
