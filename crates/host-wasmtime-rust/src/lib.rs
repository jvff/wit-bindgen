...
pub mod rt {
    ...
    pub trait RawMem {
        ...
        fn store_many<T: Endian>(&mut self, offset: i32, vals: &[T]) -> Result<()>;
    ...
    impl RawMem for [u8] {
        fn store<T: Endian>(&mut self, offset: i32, val: T) -> Result<()> {
            let mem = self
                ...
                .ok_or_else(|| anyhow!("out of bounds write"))?;
        ...
        fn load<T: Endian>(&self, offset: i32) -> Result<T> {
            let mem = self
                ...
                .ok_or_else(|| anyhow!("out of bounds read"))?;
            Ok(Le::from_slice(mem)[0].get())
        }
    }

    pub fn char_from_i32(val: i32) -> Result<char> {
        core::char::from_u32(val as u32).ok_or_else(|| anyhow!("char value out of valid range"))
    }
    ...
    pub fn validate_flags<T, U>(bits: T, all: T, name: &str, mk: impl FnOnce(T) -> U) -> Result<U>
    where
    ...
    pub fn get_func<T>(caller: &mut Caller<'_, T>, func: &str) -> Result<Func, anyhow::Error> {
        let func = caller
            .get_export(func)
            .ok_or_else(|| {
    ...
    pub fn get_memory<T>(caller: &mut Caller<'_, T>, mem: &str) -> Result<Memory, anyhow::Error> {
        let mem = caller
            .get_export(mem)
            .ok_or_else(|| {
    ...
    pub fn copy_slice<T: Endian>(
        ...
        let size = (len as u32)
            ...
            .ok_or_else(|| anyhow!("array too large to fit in wasm memory"))?;
        let slice = memory
            .data(&store)
            .get(base as usize..)
    ...
    impl IndexSlab {
        ...
        pub fn get(&self, slab_idx: u32) -> Result<ResourceIndex> {
            match self.slab.get(slab_idx) {
                Some(idx) => Ok(*idx),
                None => Err(anyhow!("invalid index specified for handle")),
            }
        }

        pub fn remove(&mut self, slab_idx: u32) -> Result<ResourceIndex> {
    ...
    impl ResourceSlab {
        ...
        pub fn clone(&mut self, idx: ResourceIndex) -> Result<()> {
            let resource = self.slab.get_mut(idx.0).unwrap();
            resource.refcnt = match resource.refcnt.checked_add(1) {
                Some(cnt) => cnt,
                None => return Err(anyhow!("resource index count overflow")),
            };
            Ok(())
        }
