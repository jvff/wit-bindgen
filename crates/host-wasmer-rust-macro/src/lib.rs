...
use wit_bindgen_gen_host_wasmer_rust::Async;
...
impl Parse for Opts {
    fn parse(input: ParseStream<'_>) -> Result<Opts> {
        ...
        let mut opts = wit_bindgen_gen_host_wasmer_rust::Opts::default();
...
enum ConfigField {
    Interfaces(Vec<Interface>),
    Async(wit_bindgen_gen_host_wasmer_rust::Async),
    CustomError(bool),
}
        ...
        if l.peek(kw::src) {
            ...
            if !cfg!(feature = "async") {
                return Err(input.error(
                    "async support not enabled in the `wit-bindgen-host-wasmer-rust` crate",
                ));
            }
